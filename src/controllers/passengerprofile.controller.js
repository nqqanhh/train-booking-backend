...
const getPassenger = async (req, res) => {
  try {
    ...
    const passengerList = await PassengerProfiles.find({
      where: {
        user_id: user.id,
      },
    });
...
const createPassengerProfile = async (req, res) => {
  try {
    ...
    await PassengerProfiles.create(newPassenger);
    res.status(200).json({
...
const updatePassengerProfile = async (req, res) => {
  try {
    ...
    const { fullName, id_no, dob, phone } = req.body;
    const pickedPassenger = await PassengerProfiles.findOne({
      where: {
    ...
    await PassengerProfiles.update(
      {
...
const deletePassengerProfile = async (req, res) => {
  try {
    ...
    await PassengerProfiles.delete({ where: { id: pickedPassenger.id } });
    res.status(200).json({
